...
Logging is done to the local folder %APPDATA%\VISBO\visbo-connect\visboconnect.log (APPDATA = C:\Users\<username>\AppData\Roaming).
...
The josn file has two parameters defined. excelExe and speSheet. The example content looks like this:
...
The solution is located in the VISBO GutHub under https://bitbucket.org/visboAtlassian/visbo-connect/src/development/
It contains all the components for the creation of an setup.exe build for Visbo Connect.

After the cloning of the GitHub solution to your local folder the program can be start for testing with a call like this:
